...
class ProcessHandler(
    ...
    override fun handleMethodCall(call: MethodCall, result: MethodChannel.Result): Boolean {
        when (call.method) {
            ...
            "isNativeBootstrapComplete" -> {
                result.success(NativeRuntimeManager(context).isInstalled())
                return true
            }
            "isNativeExecAllowed" -> {
                result.success(NativeRuntimeManager(context).isExecAllowed())
                return true
            }
            "getNativeBootstrapStatus" -> {
                result.success(NativeRuntimeManager(context).status())
                return true
            }
            "runNativeBootstrap" -> {
                executor.execute {
                    try {
                        val output = NativeRuntimeManager(context).bootstrap { line ->
                            activity.runOnUiThread {
                                try {
                                    GatewayService.logSink?.success(line)
                                } catch (_: Exception) {}
                            }
                        }
                        ...
                        activity.runOnUiThread {
                            result.error("NATIVE_BOOTSTRAP_ERROR", e.message, null)
                        }
                    }
                }
                return true
            ...
            "runNativeCommand" -> {
                ...
                if (command != null) {
                    executor.execute {
                        try {
                            val output = NativeRuntimeManager(context).runCommand(command, timeout)
                            activity.runOnUiThread { result.success(output) }
                        } catch (e: Exception) {
                            activity.runOnUiThread {
                                result.error("NATIVE_COMMAND_ERROR", e.message, null)
                            }
                        }
                    }
                } else {
            ...
            "getNativeTerminalConfig" -> {
                val nativeRT = NativeRuntimeManager(context)
                if (!nativeRT.isInstalled()) {
                    result.error("NATIVE_NOT_INSTALLED", "Native runtime not installed", null)
                } else {
                    result.success(nativeRT.getNativeTerminalConfig())
                }
                return true
            }
            "cleanupProotRootfs" -> {
                val nativeRT = NativeRuntimeManager(context)
                val cleaned = nativeRT.cleanupProotRootfs()
